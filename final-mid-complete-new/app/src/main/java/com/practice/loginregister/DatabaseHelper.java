...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    protected void InsertToMyRefrigerator(SQLiteDatabase db, String name , String[][] insert_arr){
        //insert_arr의 첫번째 열은 ingredient, 두번째 열은 period
        String id = "";
        int row = 0;
        try {
            String user_find_SQL = "SELECT id FROM usr_info WHERE name=" + name + ";";
            cursor = db.rawQuery(user_find_SQL, null);
            while (cursor.moveToNext()){
                id = cursor.getString(0);       //현재 로그인한 유저의 id키값
            }
            for(int i=0; i<insert_arr.length; i++){     //행 개수(재료 수)만큼 반복
                String SQL = "INSERT INTO refrigerator" + id + "(ingredient, period) VALUES('" + insert_arr[i][0] +
                        "', '" + insert_arr[i][1] + "');";      //일단 유통기한으로 넣도록 구현. 소비기한으로 어떻게 바꿀지
